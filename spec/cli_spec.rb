...
describe 'DocFace::Cli' do
  ...
  describe 'cli commands' do
    ...
    it 'should return directory' do
      ...
      response.should eq({:dir=>["~/test"], :output=>nil, :title=>nil, :description=>nil, :wide=>false, :version=>false, :help=>false, :dir_given=>true})
    end

    it 'should return all options' do
      ARGV.push("-d", "~/test", "-o", "~/my_output", "-t", "something", "-D", "some random description")
      response = DocFace::Cli.opts
      response.should eq({:dir=>["~/test"], :output=>"~/my_output", :title=>"something", :description=>"some random description", :wide=>false, :version=>false, :help=>false, :dir_given=>true, :output_given=>true, :title_given=>true, :description_given=>true})
    end
  end
end
